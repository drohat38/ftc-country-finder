...
Cities with no Eventbrite link yet show as "Coming soon" with a Notify me button instead of disappearing or sitting as dead text. The embed is a fixed 1,150 px tall (1,350 on phones); the city list scrolls inside it, so the Wix page stays easy to scroll on a phone.
